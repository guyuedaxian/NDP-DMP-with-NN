...
def load_model(model_path, root_path=None):
    ...
    state = torch.load(os.path.join(model_path, 'net_parameters'), map_location=torch.device('cpu'))
...
class CNNEncoderDecoderNet(torch.nn.Module):
    def __init__(self,
        ...
        if pretrained_cnn_model_path:
            try:
                self.cnn_model.load_state_dict(torch.load(pretrained_cnn_model_path, map_location=torch.device('cpu')))
            except:
                self.cnn_model.load_state_dict(torch.load(os.path.join('../', pretrained_cnn_model_path), map_location=torch.device('cpu')))
...
class FullCNNEncoderDecoderNet(torch.nn.Module):
    def __init__(self,
        ...
        # Initialize MNIST CNN model
        self.cnn_model = MNISTNet()
        print('------',pretrained_cnn_model_path)
        # Load the pretrained weights
        if pretrained_cnn_model_path:
            try:
                self.cnn_model.load_state_dict(torch.load(pretrained_cnn_model_path, map_location=torch.device('cpu')))
