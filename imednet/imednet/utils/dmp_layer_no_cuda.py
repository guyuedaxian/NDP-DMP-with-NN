...
def integrate(data, w, y0, dy0, goal, tau):
    ...
    for i in range(0, int(data[2].item())-1):
        ...
        fx = torch.mv(w*x, psi) / torch.sum(psi) # equation 10

        dx = (-data[4].item() * x) / tau   # equation 9
        dz = data[5].item() * (data[5].item() / 4 * (goal - y) - z) + fx  # equation 7
        dy = z    # equation 8
...
class DMPParameters():
    def __init__(self, N, tau, dt, Dof, scale):
        ...
        self.y_max = torch.from_numpy(scale.y_max).float()
        self.y_min = torch.from_numpy(scale.y_min).float()
        ...
        xy = torch.tensor([self.y_max, self.y_min])
        # xy = torch.cat((torch.from_numpy(self.y_max), torch.from_numpy(self.y_min)), 0)
        scale_tensor = torch.cat((self.K, self.x_min[1:55], self.x_max[1:55], xy), 0)
        print('========')
