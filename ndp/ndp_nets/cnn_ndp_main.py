...
from ndp.mnist_cnn.cnn_net.cnn import CNNNet
...
class NdpCNN(nn.Module):
    ...
    def forward(self, input, y0, return_preactivations=False):
        # [batch size, 28, 28]
        # [batch size, 28, 28] # [batch size, 2]
        x = input  # [100, 28, 28]
        x = x.view(-1, 1, self.imageSize, self.imageSize)  # [batch size, 1, 28, 28]
        # first conv
        x = F.relu(self.pt.conv1(x))  # [batch size, 20, 24, 24]
        x = F.max_pool2d(x, 2, 2)  # [batch size, 20 ,12, 12]
        # second conv
        x = F.relu((self.pt.conv2(x)))  # [batch size, 50, 8, 8]
        x = F.max_pool2d(x, 2, 2)  # [batch size, 50, 4, 4]

        # full connection layer of CNN
        x = x.view(-1, 4 * 4 * 50)  # [batch size, 800]

        activation_fn = self.hidden_activation
        #
        x = self.pt.fc1(x)  # [batch size, 500]
        x = activation_fn(x)  # [batch size, 500]

        # many liner layer and through the activation function
        for layer in self.middle_layers:
            # [batch size, 100] [batch size, 500] [batch size, 128] [batch size, 64]
            x = activation_fn(layer(x))

        # last special fc
        output = self.last_fc(x) * 1000  # [batch size, 64]

        # [batch size, 2] -> # [batch size * 2]
        y0 = y0.reshape(-1, 1)[:, 0]
        dy0 = torch.zeros_like(y0) + 0.01  # [batch size * 2], [0.01000, 0.0100, ...]

        # get the [batch size * 2, 301], [batch size * 2, 301] [batch size * 2, 301]
