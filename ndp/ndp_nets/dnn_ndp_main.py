...
from ndp.mnist_cnn.cnn_net.dnn import DNNNet


class NdpDNN(nn.Module):
    """
        NdpDNN network
    """
    ...
    def forward(self, input, y0, return_preactivations=False):
        ...
        x = x.view(-1, 1 * 28 * 28)     # [100, 784]

        activation_fn = self.hidden_activation
        x = self.pt.fc1(x)  # [100, 200]
        x = activation_fn(x)  # [100, 200]

        # many liner layer and through the activation function
        for layer in self.middle_layers:
            # [200, 100] [100, 200] [200, 128] [128, 64]
