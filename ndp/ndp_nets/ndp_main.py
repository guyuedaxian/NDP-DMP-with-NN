...
# @Time        : 2021/6/16 2:20
# @Author      : linksdl
# @ProjectName : acs-project-msc_project_dmp
# @File        : ndp_main.py
...
from ndp.mnist_cnn.cnn_net.mnist_cnn import CNN
...
class NdpCNN(nn.Module):
    ...
    def forward(self, input, y0, return_preactivations=False):
        ...
        # second conv
        x = F.relu((self.pt.conv2(x))) # [100, 50, 8, 8]
        x = F.max_pool2d(x, 2, 2)      # [100, 50, 4, 4]

        # full connection layer of CNN
        x = x.view(-1, 4 * 4 * 50)     # [100, 800]
