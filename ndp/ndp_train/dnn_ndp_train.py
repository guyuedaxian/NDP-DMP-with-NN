...
parser.add_argument('--name', type=str, default='dnn-ndp-il')
...
if dataset_name == 'smnist':
    ...
    data_stub = 'smnist'
    pre_trained = '../mnist_cnn/cnn_trained/mnist_dnn_net_' + str(neuros) +'(mnist).pt'

elif dataset_name == 'smnist-awgn':
    data_path = '../../imednet/data/s-mnist/40x40-smnist-with-awgn.mat'
    data_stub = 'smnist_awgn_9_5_snr'
    pre_trained = '../mnist_cnn/cnn_trained/mnist_dnn_net_' + str(neuros) + '(mnist-awgn).pt'

elif dataset_name == 'smnist-mb':
    data_path = '../../imednet/data/s-mnist/40x40-smnist-with-motion-blur.mat'
    data_stub = 'smnist_mb'
    pre_trained = '../mnist_cnn/cnn_trained/mnist_dnn_net_' + str(neuros) + '(mnist-motion-blur).pt'
    ...
    pre_trained = '../mnist_cnn/cnn_trained/mnist_dnn_net_' + str(neuros) + '(mnist-reduced-contrast-and-awgn).pt'
...
# the ndpcnn model
ndpn = NdpDNN(T=T, l=1, N=N, pt=pre_trained, state_index=np.arange(2))
...
for epoch in range(num_epochs):
    ...
    torch.save(ndpn.state_dict(), model_save_path + '/dnn-model.pt')
